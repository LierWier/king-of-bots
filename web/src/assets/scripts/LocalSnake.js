...
export default class LocalSnake extends AcGameObject {
    ...
    render() {
        ...
        if (this.status === "die") {
            ...
            if(!this.isCommit) {
                if (this.id === 0) {
                    this.store.commit("updateAStatus", "die")
                } else if (this.id === 1) {
                    this.store.commit("updateBStatus", "die")
                }
                this.store.commit("updateGameStatus", "over")
                this.isCommit = true
            }
        }
